...
const rules = {
    ...
    duplicate: (value) => !props.library.admins?.some(admin => admin.email === value + "@gmail.com") || "L'administrateur est déjà ajouté.",
...
function send() {
    sending.value = true;
    addAdmin(props.library.id, email.value + "@gmail.com")
        .then(library => {
        ...
        .catch(err => {
            ...
            if (err.message.includes('[401]')) {
                ...
                globalEmitter.emit('error', { message: "Il semblerait que l'administrateur soit déjà ajouté." });
