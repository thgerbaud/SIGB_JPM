<template>
    ...
    <ConfirmDeletionDialog v-model="confirmDeleteDialog" title="Supprimer l'exemplaire" okText="Supprimer" cancelText="Annuler"
    ...
    <tr>
        ...
        <td v-if="library.isAdmin" class="d-flex align-center">
            <v-tooltip text="Modifier l'exemplaire">
                <template v-slot:activator="{ props }">
                    <v-icon v-bind="props" icon="mdi-pencil" class="mr-4" @click="editCopyModal = true"></v-icon>
                </template>
            </v-tooltip>
            <v-tooltip text="Supprimer l'exemplaire">
                <template v-slot:activator="{ props }">
                    <v-icon v-bind="props" icon="mdi-delete" color="error" @click="confirmDeleteDialog = true"></v-icon>                </template>
            </v-tooltip>
        </td>
    </tr>
</template>
